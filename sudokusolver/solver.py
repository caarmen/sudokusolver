...
class Sudoku:
    ...
    def resolve_unambiguous_cells(self):
        ...
        Then repeat.
        Stop when all the empty cells have at least 2 possible numbers that can fit, based on the numbers of their
        row, group, and square
        """
    ...
    def is_valid(self) -> bool:
        ...
        are_rows_valid = all(set(row) == full_group for row in self.data)
        are_cols_valid = all(set(col) == full_group for col in numpy.transpose(self.data))
        are_squares_valid = all(set(square) == full_group for square in squares)
